...
namespace KMBOXNET {
	...
	bool isKeyPressed(int key) {
		switch (key) {
		case LMB: {
			int result = kmNet_monitor_mouse_left();
			return result > 0;
		}
		case RMB: {
			int result = kmNet_monitor_mouse_right();
			return result > 0;
		}
		case MMB: {
			int result = kmNet_monitor_mouse_middle();
			return result > 0;
		}
		default: {
			int kbResult = kmNet_monitor_keyboard(static_cast<short>(key));
			return kbResult > 0;
		}
		}
	}
	...
	bool ConnectKMBox(const char* uuid, const char* ip, int port) {
		...
		return kmNet_init((char*)ip, portStr, (char*)uuid) == 0;
	}
}
...
static unsigned char key[16] = { 0 };	//������Կ
static HANDLE m_hMutex_lock = NULL;	    //���̻߳�����
extern unsigned int xbox_mac;			//xboxAPIҲ��Ҫmac
#pragma pack(1)
typedef struct {
...
extern "C" {
	int kmNet_monitor_keyboard(int key);
	int kmNet_monitor_mouse(int key);  // only one mouse function!
	int kmNet_init(char* ip, char* port, char* uuid);
}
...
int NetRxReturnHandle(client_tx* rx, client_tx* tx)		 //���յ�����
{
	...
	ret=0;//ֻҪ�л������Ϊִ��ok��������ݰ��������ա� ���粻�ɴ���ա����ݰ�˳�򲻶Է��ա� ����������̫��ȷ��������绷�������롣���ᵼ������ϵͳ������
	return ret;				//û�д��󷵻�0
}
...
int kmNet_init(char* ip, char* port, char* mac)
{
	...
	// Set socket timeout to prevent blocking (1 second timeout)
	...
	tx.head.mac = StrToHex(mac, 4);		 //���ӵ�mac �̶� UUID
	xbox_mac = tx.head.mac;				 //��¼macֵ
	...
	key[0] = tx.head.mac >> 24; key[1] = tx.head.mac >> 16;//���ü�����Կ
	key[2] = tx.head.mac >> 8; key[3] = tx.head.mac  >> 0; //���ü�����Կ
	...
	if (err == SOCKET_ERROR) {
		...
		if (error == WSAETIMEDOUT) {
			// Timeout - kmbox not connected, return error code
			return -1;
		}
	}
...
�Լ�д�켣�ƶ�ʱʹ�ô˺�����
����ֵ��0����ִ�У�����ֵ�쳣��
�˺����Ǵ����ܹ��ܵģ����Ա�֤ͬһ���ƶ�ָ���������ݰ����ݶ���һ�����޷�ͨ�����緢��ץ�����������ӡ�
*/
...
{	int err; client_tx  tx_enc = {0};
	...
	my_encrypt((unsigned char*)&tx_enc, key);														 //���ܷ�������
	...
	err = recvfrom(sockClientfd, (char*)&rx, 1024, 0, (struct sockaddr*)&sclient, &clen);
	ReleaseMutex(m_hMutex_lock);
	return 0;//ֻҪ�յ��������Ϊ����OK �����ٺ��ӵļ��ܹ��̡� ��Լʱ��
}
...
{	int i; client_tx  tx_enc = { 0 };
	...
	{//�����  
		...
		{
			if (softkeyboard.button[i] == vk_key)
				goto KM_enc_down_send;// ���������Ѿ���vk_key ֱ�ӷ��;���
		}
		//��������û��vk_key 
		for (i = 0; i < 10; i++)//�������е����ݣ���vk_key���ӵ�������
		{
...
//ʹ�ܺ��ӵ�Ӳ����������
/*
type  :0:����������  1������׷������,2:������ʵʱ ��3:RM-RT
value ��С�ڻ��ߵ���0��ʾ�ر�Ӳ�������������ܡ�����0��ʾ����Ӳ�������������ܡ���ֵԽ��������Խƽ��������ʱԽ�ߡ��Ƽ�ȡֵ��Χ16��50֮�䡣�����Ե�100.
*/
int kmNet_Trace(int type, int value)
{
	...
	WaitForSingleObject(m_hMutex_lock, 2000);
	tx.head.indexpts++;				//ָ��ͳ��ֵ
	tx.head.cmd = cmd_trace_enable;	//ָ��
	tx.head.rand = type << 24 | value;			//���ֵ
