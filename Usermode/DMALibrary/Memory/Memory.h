...
private:
	...
	static inline std::atomic<bool> s_verboseFailures { false };
	...
	*/
	...
	//shared pointer
	std::shared_ptr<c_keys> m_input_keys;
	c_registry registry;
...
public:
	...
	static void SetVerboseFailures(bool enabled) {
		s_verboseFailures.store(enabled, std::memory_order_relaxed);
	}

	static bool VerboseFailures() {
		return s_verboseFailures.load(std::memory_order_relaxed);
	}
	...
	* @return key class
	*/
	c_keys* GetKeyboard() { return m_input_keys.get(); }
	...
	*/
	 ...
	 */
	uint64_t FindSignature(const char* signature, const char* mask, uint64_t range_start, uint64_t range_end, int PID = 0);
	...
	*/
	...
	/**
	 * \brief Reuses one scatter handle per process (cleared after each execute).
	 * Avoids expensive VMMDLL_Scatter_Initialize/Close per batch/frame.
	 */
	VMMDLL_SCATTER_HANDLE GetScatterHandle() const;
	...
	void AddPageOptimizedScatterRead(VMMDLL_SCATTER_HANDLE handle, uint64_t address, void* buffer, size_t size);
	...
	/*handles are thread-local via GetScatterHandle(); no shared mutable handle*/
