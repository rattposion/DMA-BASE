...
namespace ImGui
{
    ...
    // ID stack/scopes
    // Read the FAQ (docs/FAQ.md or ) for more details about how ID are handled in dear imgui.
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasTextures   = 1 << 4,   // Backend Renderer supports ImTextureData requests to create/update/destroy textures. This enables incremental texture updates and texture reloads. See  for instructions on how to upgrade your custom backend.
};
...
// See  for details and pseudo-code using this helper.
...
struct ImFontConfig
{
    ...
    ImS8            OversampleH;            // 0 (2)    // Rasterize at higher quality for sub-pixel positioning. 0 == auto == 1 or 2 depending on size. Note the difference between 2 and 3 is minimal. You can reduce this to 1 for large glyphs save memory. Read  for details.
...
namespace ImGui
{
    ...
    //-- OBSOLETED in 1.78 (from June 2020): Old drag/sliders functions that took a 'float power > 1.0f' argument instead of ImGuiSliderFlags_Logarithmic. See  for details.
